...
{
    vector< pair<cv::Mat, cv::Point>> regions = getBounds(img, display);  //Retrieve vector of Mats possibly containing letters
    vector<Letter> let;
    Mat scan;
    for(int i=0; i<regions.size(); i++) //Iterate through all retrieved Mats
    {
        if(regions[i].first.cols > 0){
            ...
            if(!res.empty() && confidence->at(0) >= min_confidence)
            {
				...
				let.at(let.size()-1).rect = Rect(regions[i].second.x - regions[i].first.cols/2
					, regions[i].second.y - regions[i].first.rows/2
					, regions[i].first.cols, regions[i].first.rows);
                if(display){
                    imshow(("R"+to_string(i)),scan); waitKey(5);
                    rectangle( img, let.at(let.size()-1).rect, Scalar(255,255,0), 4);
                }
            }
